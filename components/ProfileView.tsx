...
const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-700 overflow-hidden w-full relative pb-10 md:pb-8">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-8 px-4 md:px-0 pb-12">
        ...
        <div className="bg-white p-8 md:p-10 rounded-[3rem] border border-slate-200 shadow-xl">
           ...
           <div className="space-y-4">
               ...
               <div className="flex flex-wrap gap-2">
                 {DAYS.map((day, idx) => (
                   <button 
                     ...
                     className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${iterationDay === idx ? 'bg-indigo-600 text-white shadow-lg scale-105' : 'bg-slate-50 text-slate-400 hover:bg-slate-100 active:bg-slate-200'}`}
                   >
                     {day.replace('星期', '')}
                   </button>
                 ))}
             ...
             <p className="text-[10px] text-indigo-400 italic bg-indigo-50 p-4 rounded-2xl leading-relaxed">
      ...
      {isEditing && (
        <footer className="mt-4 mb-4 flex items-center justify-center space-x-4 shrink-0 animate-in slide-in-from-bottom-4 px-4">
          <button onClick={handleCancel} className="px-8 py-4 rounded-3xl text-sm font-black uppercase text-slate-400 hover:text-slate-600">取消 CANCEL</button>
