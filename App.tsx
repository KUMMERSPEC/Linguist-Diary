...
const App: React.FC = () => {
  ...
  const handleStartSmartReview = (mode: 'single' | 'combo') => {
    ...
    while (i < sorted.length && finalQueue.length < 10) {
      ...
      // Try to find 1-2 more words to form a combo if mastery is low and mode is combo
      if (mode === 'combo' && (v.mastery || 0) < 3) {
        ...
        <VocabPractice 
          ...
          onNextInQueue={() => { 
            if (queueIndex < practiceQueue.length - 1) { 
              ...
              setSelectedVocabForPracticeId(practiceQueue[queueIndex + 1].join(',')); 
            } else { 
              setView('vocab_list'); 
              setIsPracticeActive(false);
            } 
          }} 
          nextVocabId={queueIndex < practiceQueue.length - 1 ? practiceQueue[queueIndex + 1].join(',') : undefined}
        />
      )}
